...
- **browser-verifier** (new): Uses the agent-browser CLI (`pnpm exec agent-browser open ...; snapshot -i` for @eN refs; fill/click/wait) to perform real functional verification of the live staff and public UIs over portless .localhost. Provides ground-truth "did the UI actually work for a user?" data (including after GraphQL mutations). Scheduled periodically by the orchestrator and feeds results into agent-evaluator. See the `verify-ui-with-browser` skill.
  ...
  **Harness reality note (dev server lifetime):** pnpm-launched dev servers (api, staff, public) in this environment are short-lived (~60s before harness kill, exit 143 / SIGTERM). Browser-verifier runs (and any orchestration involving live UI verification) **must** use proven patterns: background `run_terminal_command`, persistent `monitor`, `get_command_or_subagent_output` polling, aggressive `pkill` before re-launches, and direct port fallbacks. On repeated server errors (after 2 attempts), **immediately fallback** to terminal hygiene test (`bun -e '...runHygieneTest()'` on deposit-calculator) + curl GraphQL checks for the target case (e.g. high-deposit 20000 BankBuckets apply), explicitly reporting the env limitation while still delivering calc/apply verification evidence (PASSED hygiene counts as functional confirmation). This is detailed in `.grok/personas/browser-verifier.toml`, `.grok/skills/verify-ui-with-browser/SKILL.md`, and the portable equivalents. The `agent-evaluator` tracks env/harness resilience (cancelled tool counts, doom-loop duration) as a first-class data point for refinements. Never assume long-lived servers.
