...
- **browser-verifier** (new): Uses the agent-browser CLI (`pnpm exec agent-browser open ...; snapshot -i` for @eN refs; fill/click/wait) to perform real functional verification of the live staff and public UIs over portless .localhost. Provides ground-truth "did the UI actually work for a user?" data (including after GraphQL mutations). Scheduled periodically by the orchestrator and feeds results into agent-evaluator. See the `verify-ui-with-browser` skill.
  - For **public UI / projections / Epic-5+** scopes (Next Paycheck live preview, goals grid, tree deltas, teaser): always spawn on grok-4-fast (or higher) for @e fidelity. Include "public" or "Epic-5" in the task title. Use the full resilient protocol (bg/monitor/poll/pkill/direct + hygiene/curl fallback per persona). Report public @e coverage count + hygiene passes; this drives the "Public browser @e coverage" metric in evaluator reports. (Codified from agent-evaluator proposal after successful public Epic-5 verification.)

  **Harness reality note (dev server lifetime):** pnpm-launched dev servers (api, staff, public) in this environment are short-lived (~60s before harness kill, exit 143 / SIGTERM). This is the #1 environmental constraint for any work involving live UI verification or long agent sessions.

  **For 30m+ work or 1h+ analysis tests (e.g. the bigger BankBuckets cycle test):**
  - Prefer the dedicated guardian: `pnpm dev:agent` (implemented in `scripts/guard-dev-servers.js`). It launches direct-mode servers (api:4000, public:5173, staff:5174, no portless proxy) with auto-restart loops and detached children. This is the current best effort to extend practical server lifetime inside the harness.
  - Immediately attach persistent `monitor` + polling for health (curl on ports + GraphQL).
  - Browser-verifier (and any orchestration) **must** follow the full resilience protocol in `.grok/personas/browser-verifier.toml` and the `verify-ui-with-browser` skill:
    - background launch of guardian/individual directs
    - aggressive pkill + relaunch on deaths
    - direct port fallbacks (avoid portless registration fights)
    - After 2 repeated errors: **immediate fallback** to terminal hygiene test + curl (never loop on server launches)
  - Always report the limitation explicitly. Hygiene (PASSED on realistic BankBuckets high-deposit cases) + curl + client @e evidence counts as real functional confirmation.
  - The `agent-evaluator` treats harness resilience data (kill counts, fallback usage, doom-loop avoidance) as first-class metrics.

  Full details and the exact fallback protocol live in the browser-verifier persona, the verify skill, and AGENTS.md notes. Never assume long-lived servers.

**Spawn prompt hygiene for high-context roles (muse, muse-eyes, and similar):** When spawning these (or other large-context personas), ALWAYS use ultra-condensed prompts that reference the persona file rather than embedding full instructions: "Act as the persona defined in .grok/personas/NAME.toml (use read_file or open tools to load it fresh if you have not already in context); do not paste, embed, quote, or include the full instructions, toml body, or long description in the subagent prompt, title, or summary. Ultra-focused task only: [1-2 sentences]. Begin work immediately with explicit first tool actions: 1. list_dir [relevant path] NOW. 2. [next]." This prevents the recurring turn-1/0-tool high-token stuck pattern observed on long persona embeds (quantified in agent-evaluator checkpoints). Update this guidance when the pattern is retired.

**Long-running dev servers for extended tests (1h+ analysis, repeated verification):** The harness kills most background dev tasks after ~60s. For any significant uninterrupted work:
- Launch servers with `pnpm dev:agent` (the guardian in `scripts/guard-dev-servers.js` — direct ports + restart loops).
- Attach persistent `monitor` + polling right away.
- Browser-verifier and orchestration must follow the full resilience protocol (see browser-verifier persona + verify skill). After 2 failures, fall back to hygiene test + curl and report the limitation. Hygiene PASSED + artifacts count as real evidence.
